<template>
  <v-container>
    <v-card
      class="mx-auto"
      max-width="400"
      v-if="$store.state.user.users.length > 0"
    >
      ...
      <v-card-text>
        <v-row align="center" class="mx-0">
          <div class="grey--text">
            {{ $store.state.user.users[userIndex].turma }}
          </div>
        </v-row>
      ...
      <v-card-actions class="px-5">
        <v-row class="my-2 justify-center">
          <v-col cols="3">
            <v-btn large @click="match()" elevation="5">
              <v-icon color="error">mdi-thumb-down</v-icon></v-btn
            >
          </v-col>
          <v-col cols="3">
            <v-btn @click="match()" large elevation="5">
    ...
    <v-card class="mx-auto px-4" max-width="400" v-else>
      ...
      <v-card-text>
        <v-row>
          <div class="my-3">
            Aproveite suas combinações em "Combinações" no Menu ou volte mais
            tarde
          </div>
        </v-row>
      </v-card-text>
    ...
    <match-overlay
      @ifriends_next="nextUser"
      :overlay="overlays.match"
      :user="$store.state.user.users[userIndex]"
    />
  </v-container>
</template>
...
export default Vue.extend({
  ...
  methods: {
    setProfilePicture() {
      const link = `${url.API_URL}/uploads/${
        this.$store.state.user.users[this.userIndex].foto
      }`;
      return (
        link ??
    ...
    async match() {
      ...
      if (
        this.$store.state.user.users.length != index ||
        this.$store.state.user.users.length == 1
      ) {
        const id = this.$store.state.user.users[this.userIndex].id;

        const origem_id = this.$store.state.user.id;
        const token = this.$store.state.user.token;

        const request = new MatchRequest(token, origem_id, id);
    ...
    async load(data) {
      ...
      console.log(
        "userIndex: " +
          this.userIndex +
          ", Users Length: " +
          this.$store.state.user.users.length
      );
    },
  },
  components: {
